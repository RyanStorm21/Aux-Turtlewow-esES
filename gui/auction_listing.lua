...
local TIME_LEFT_STRINGS = {
	aux.color.red'30m', -- Corto (Las abreviaciones como '30m', '2h' son comunes y se entienden bien)
	aux.color.orange'2h', -- Medio
	aux.color.yellow'8h', -- Largo
	aux.color.blue'72h', -- Muy Largo
}
...
M.search_columns = {
    {
        ...
        cmp = function(record_a, record_b, desc) -- Comparador para ordenar
    ...
    },
    {
        ...
        width = .035,
        align = 'CENTER',
        fill = function(cell, record) -- Rellena la celda
...
M.auctions_columns = {
    ...
    {
        title = 'Pujador', -- Máximo Pujador
        width = .21,
        align = 'CENTER',
        fill = function(cell, record)
            cell.text:SetText(record.high_bidder or aux.color.red 'Sin Pujas')
        end,
        cmp = function(record_a, record_b, desc)
...
M.bids_columns = {
    ...
    },
    {
        ...
        fill = function(cell, record)
            local status
            if record.high_bidder then
                status = aux.color.yellow'Máx. Puja'
            else
                status = aux.color.red'Superado'
            end
...
local methods = {
    ...
    OnEnter = function()
        ...
        if rt.expanded[this.expandKey] then
            ...
            GameTooltip:AddLine('Doble clic para contraer este objeto y mostrar solo la subasta más barata.', 1, 1, 1, true)
            GameTooltip:Show()
        elseif this.expandable then
            GameTooltip_SetDefaultAnchor(GameTooltip, UIParent)
            GameTooltip:AddLine('Doble clic para expandir este objeto y mostrar todas las subastas.', 1, 1, 1, true)
            GameTooltip:Show()
        end
