...
local UNKNOWN = GRAY_FONT_COLOR_CODE .. 'N/D' .. FONT_COLOR_CODE_CLOSE
...
function M.extend_tooltip(tooltip, link, quantity)
    ...
    if item_info then
        ...
        if getn(distribution) > 0 then
            if settings.disenchant_distribution then
                tooltip:AddLine('Se desencanta en:', aux.color.tooltip.disenchant.distribution())
            ...
            if settings.disenchant_value then
                ...
                tooltip:AddLine('Desencantar: ' .. (disenchant_value and money.to_string2(disenchant_value) or UNKNOWN), aux.color.tooltip.disenchant.value())
            end
        end
    end
    if settings.merchant_buy then
        ...
        if price then
            tooltip:AddLine('Compra de Vendedor ' .. (limited and '(limitado): ' or ': ') .. money.to_string2(price * quantity), aux.color.tooltip.merchant())
        end
    end
    if settings.merchant_sell then
        ...
        if price ~= 0 then
            tooltip:AddLine('Vendedor: ' .. (price and money.to_string2(price * quantity) or UNKNOWN), aux.color.tooltip.merchant())
        end
    end
    ...
    if auctionable then
        if settings.value then
            tooltip:AddLine('Valor: ' .. (value and money.to_string2(value * quantity) or UNKNOWN), aux.color.tooltip.value())
        end
        if settings.daily  then
            local market_value = history.market_value(item_key)
            tooltip:AddLine('Hoy: ' .. (market_value and money.to_string2(market_value * quantity) .. ' (' .. gui.percentage_historical(aux.round(market_value / value * 100)) .. ')' or UNKNOWN), aux.color.tooltip.value())
        end
    end
