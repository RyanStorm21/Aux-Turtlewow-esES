...
function update_search_listings()
	...
	for i = 1, getn(favorite_searches) do
		...
		tinsert(favorite_search_rows, T.map(
			'cols', T.list(T.map('value', search.auto_buy and aux.color.red'C' or search.auto_bid and aux.color.red'P' or ''), T.map('value', name)),
			'search', search,
			'index', i
		))
	end
	...
	OnClick = function(st, data, _, button)
		...
		elseif button == 'RightButton' then
			...
			elseif st == favorite_searches_listing then
				...
				gui.menu(
					(auto_buy and 'Desactivar' or 'Activar') .. ' Compra Automática', function() if auto_buy then data.search.auto_buy = nil else enable_auto_buy(data.search) end u() end,
					(auto_bid and 'Desactivar' or 'Activar') .. ' Puja Automática', function() if auto_bid then data.search.auto_bid = nil else enable_auto_bid(data.search) end u() end,
					'Mover Arriba', function() move_up(favorite_searches, data.index); u() end,
					'Mover Abajo', function() move_down(favorite_searches, data.index); u() end,
					'Eliminar', function() tremove(favorite_searches, data.index); u() end
				)
			end
		end
	end,
	...
	for _, search in favorite_searches do
		if search.auto_buy then
			...
			if queries then
				...
				aux.print('Filtro de compra automática inválido:', error)
			end
		end
	end
	...
	for _, search in favorite_searches do
		if search.auto_bid then
			...
			if queries then
				...
				aux.print('Filtro de puja automática inválido:', error)
			end
		end
	end
	return function(record)
...
function add_favorite(filter_string)
	...
	if queries then
		...
		aux.print('Filtro inválido:', error)
	end
end
...
function enable_auto_buy(search)
	...
	if queries then
		if getn(queries) > 1 then
			aux.print('Error: Compra Automática no admite multi-consultas')
		elseif aux.size(queries[1].blizzard_query) > 0 and not filter_util.parse_filter_string(search.filter_string).blizzard.exact then
			aux.print('Error: Compra Automática no admite filtros de Blizzard')
		else
...
function enable_auto_bid(search)
	...
	if queries then
		if getn(queries) > 1 then
			aux.print('Error: Puja Automática no admite multi-consultas')
		elseif aux.size(queries[1].blizzard_query) > 0 and not filter_util.parse_filter_string(search.filter_string).blizzard.exact then
			aux.print('Error: Puja Automática no admite filtros de Blizzard')
		else
