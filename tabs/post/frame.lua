...
do
    ...
    label:SetText('Mostrar objetos ocultos')
...
bid_listing:SetColInfo{
    {name='Subastas', width=.17, align='CENTER'},
    {name='Tiempo\nRestante', width=.11, align='CENTER'},
    {name='Cantidad\nLote', width=.11, align='CENTER'},
    {name='Puja Subasta\n(por objeto)', width=.4, align='RIGHT'},
...
do
    ...
    slider.label:SetText('Tamaño del Lote')
...
do
    ...
    slider.editbox:SetNumeric(true)
    slider.label:SetText('Cantidad de Lotes')
...
do
    ...
    local label = gui.label(checkbox, gui.font_size.small)
    label:SetPoint('LEFT', checkbox, 'RIGHT', 4, 1)
    label:SetText('Ocultar este objeto')
...
do
    ...
    do
        ...
        label:SetText('Precio Inicial Unitario')
    end
    do
...
do
    ...
    do
        local label = gui.label(editbox, gui.font_size.small)
        label:SetPoint('BOTTOMLEFT', editbox, 'TOPLEFT', -2, 1)
        label:SetText('Precio de Compra Unitario')
    end
    do
        local label = gui.label(editbox, 14)
        label:SetPoint('LEFT', editbox, 'RIGHT', 8, 0)
...
function aux.handle.LOAD()
	if not aux.account_data.post_bid then
		...
		buyout_listing:SetColInfo{
			{name='Subastas', width=.15, align='CENTER'},
			{name='Tiempo Restante', width=.15, align='CENTER'},
			{name='Cantidad Lote', width=.15, align='CENTER'},
			{name='Compra Subasta (por objeto)', width=.4, align='RIGHT'},
			{name='% Valor Hist.', width=.15, align='CENTER'},
		}
	end
end